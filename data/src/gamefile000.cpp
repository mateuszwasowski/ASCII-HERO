...
#include "../hdrs/gamefile001.h"
#include "../hdrs/gamefile002.h"

using namespace std;

string intToStr3(int n)
{
     string tmp, ret;
     if(n < 0) {
      ret = "-";
...
void gamefile000(){
	...
	map.open( "data/maps/map001.txt", ios::in | ios::out );
		...
		while ( key != 8 ){  //backspace
			...
			if ( key == 'w' || key == 'W'){
				...
				if ( cord_y_x[hero_y-1][hero_x] == '-' || cord_y_x[hero_y-1][hero_x] == '*' || cord_y_x[hero_y-1][hero_x] == '|' ){
					...
					if( temp_key == '*' ){
						...
						cord_y_x[20] = "My father built this road all by himself!";
			...
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ KLAWISZ S 
				...
				if ( cord_y_x[hero_y+1][hero_x] == '-' || cord_y_x[hero_y+1][hero_x] == '*' || cord_y_x[hero_y+1][hero_x] == '|' ){
					...
					if (h.inventory != "dogtalk"){
					cord_y_x[20] = "I can't leave yet, I still have to speak to my dog!";
					}
					else {
	...
	cout << "ERROR. Can't open map001.txt file!";
	key == getch();
