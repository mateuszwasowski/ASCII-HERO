...
#include "../hdrs/gamefile004.h"
#include "../hdrs/gamefile005.h"


using namespace std;

string intToStr8(int n)
{
     string tmp, ret;
     if(n < 0) {
      ret = "-";
...
void gamefile003(){
	...
	map.open( "data/maps/map004.txt", ios::in | ios::out );
	...
	if (map.good() == true){
		...
		while ( key != 8 ){  //backspace
				...
				if ( cord_y_x[hero_y-1][hero_x] == '-' || cord_y_x[hero_y-1][hero_x] == '*' || cord_y_x[hero_y-1][hero_x] == '|' ){
					...
					if( temp_key == '*' ){
						...
						if (temp_random < 94){
							...
							cord_y_x[20] = "There might be creatures hiding in these bushes";
			...
			//++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++ KLAWISZ A
				...
				else if ( cord_y_x[hero_y][hero_x-1]==' ' ){
	...
	cout << "ERROR. Can't open map004.txt file!";
	key == getch();
